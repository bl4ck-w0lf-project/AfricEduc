...
<body class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 text-slate-800 antialiased">
  ...
  <aside id="sidebar" class="fixed left-0 top-0 z-50 flex h-full w-[260px] -translate-x-full flex-col bg-gradient-to-b from-primary to-primaryDark text-white shadow-2xl transition-transform duration-300 lg:translate-x-0">
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-6 text-sm">
      ...
      <a href="#" class="sidebar-link active flex items-center gap-3 rounded-xl px-3 py-2.5 mt-1">
      ...
      <div id="dynamic-content">
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-8">
          ...
          <div class="kpi-card rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm" style="border-left-color: #22c55e;">
            ...
            <p class="text-3xl font-bold text-slate-900">---<span class="text-lg font-normal text-slate-500"> /20</span></p>
          </div>
        ...
        <div class="flex flex-wrap justify-between gap-4 mb-6">
          ...
          <a href="#" class="action-button inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-primaryDark">
            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 4.5v15m7.5-7.5h-15"/></svg>
            Ajouter un élève
          </a>
        </div>

        <!-- Tableau des élèves (statique, prêt pour PHP) -->
            ...
            <table class="min-w-full text-left text-sm">
              ...
              <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/80 transition-all">
                  <td class="px-5 py-4"><div class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center"><svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M16 7a4 4 0 1 1-8 0 4 4 0 0 1 8 0ZM12 14a7 7 0 0 0-7 7h14a7 7 0 0 0-7-7Z"/></svg></div></td>
                  <td class="px-3 py-4 font-medium text-slate-900">--- ---</td>
                  <td class="px-3 py-4">---</td>
                  <td class="px-3 py-4 text-slate-700">--- /20 <span class="mention-badge mention-insufficient">---</span></td>
                  <td class="px-5 py-4 text-right">
                    <div class="flex justify-end gap-2">
                      <a href="#" class="text-slate-500 hover:text-primary transition" title="Voir détails">
  ...
  <!-- Modale générique (uniquement UI, aucune injection JS) -->
  ...
  <script>
    // Script uniquement pour l'UI (sidebar, toasts, fermeture modale)
    // AUCUNE injection de données ou opérations CRUD en JS
    ...
    // Initialisation et événements UI uniquement
    function init() {
      ...
      // Filtres UI seulement (affiche un toast)
      ...
      if (resetBtn) {
        resetBtn.addEventListener("click", () => {
          ...
          showToast("Filtres réinitialisés (connexion PHP requise)");
        });
      }
    }
